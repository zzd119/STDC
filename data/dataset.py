...
class TimeSeriesForecastingDataset(Dataset):
    ...
    def _check_if_file_exists(self, data_file_path: str, index_file_path: str):
        ...
        if not os.path.isfile(data_file_path):
            raise FileNotFoundError("BasicZD can not find data file {0}".format(data_file_path))
        if not os.path.isfile(index_file_path):
            raise FileNotFoundError("BasicZD can not find index file {0}".format(index_file_path))
